...
The repository keeps the pnpm workspace in `js/` and the Rust crates in `rust/`. Unless a step says otherwise, run the `pnpm` commands below from `js/`.
...
```bash
pnpm --dir js build:wasm
```

This command will build the `fedimint-client-wasm` crate and place the resulting binaries in the `js/web/wasm-bundler` and `js/web/wasm-web` folders.
...
Change into the workspace root (`cd js` from the repository root) and run the following command to install pnpm (via Corepack) and the project's dependencies. Every `pnpm` command from here on assumes that directory:
...
Documentation is crucial to helping developers of all experience levels use Fedimint Sdk. We use [VitePress](https://vitepress.dev) for the documentation site (located at `js/docs`). To start the site in dev mode, run:
